  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    try {   
      ...
      try{
        ...
        embeduserinfo.addField('**❱ Nombre:**',`<@${member.user.id}>\n\`${member.user.tag}\``,true)
        ...
        embeduserinfo.addField('**❱ Fecha de ingreso DC:**', "\`"+moment(member.user.createdTimestamp).format("DD/MM/YYYY") + "\`\n" + "`"+ moment(member.user.createdTimestamp).format("hh:mm:ss") + "\`",true)
        embeduserinfo.addField('**❱Fecha de ingreso al Servidor:**', "\`"+moment(member.joinedTimestamp).format("DD/MM/YYYY") + "\`\n" + "`"+ moment(member.joinedTimestamp).format("hh:mm:ss")+ "\`",true)
        ...
        embeduserinfo.addField('**❱ Rol Mas alto:**',`${member.roles.highest.id === message.guild.id ? 'None' : member.roles.highest}`,true)
        ...
        embeduserinfo.setAuthor("Information about:   " + user.username + "#" + user.discriminator, user.displayAvatarURL({ dynamic: true }),)
        ...
        // var userstatus = "No esta en actividad";
        // if(activity){
        //   if(activity.type === "CUSTOM_STATUS"){
        //     let emoji = `${activity.emoji ? activity.emoji.id ? `<${activity.emoji.animated ? "a": ""}:${activity.emoji.name}:${activity.emoji.id}>`: activity.emoji.name : ""}`
        //     userstatus = `${emoji} \`${activity.state || 'Not having an acitivty.'}\``
